...
UIDisconnectedBox::UIDisconnectedBox(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_OkButton = new UIButton(MESSAGEBOX::MESSAGE_DISCONNECTED_OKBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, _ViewX + 149.0f, _ViewY + 72.0f, MSGBOX_BTN_WIDTH, MSGBOX_BTN_HEIGHT);
...
UIDisconnectedBox::~UIDisconnectedBox()
{

}

void UIDisconnectedBox::Update(mu_uint64 CurrentTime)
{

}

void UIDisconnectedBox::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UIDisconnectedBox::DrawFrame()
{
	_BackFrame->Draw();
}

void UIDisconnectedBox::DrawText()
{
	FONT_NORMAL->DrawStringC2D(TEXTINDEX::MESSAGEBOX_DISCONNECTED,
...
void UIDisconnectedBox::DrawButton()
{
	_OkButton->Draw();
	_OkButton->PostDraw();
}

mu_uint32 UIDisconnectedBox::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UIDisconnectedBox::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
