...
UIBlockedFromServer::UIBlockedFromServer(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_OkButton = new UIButton(MESSAGEBOX::MESSAGE_BLOCKEDFROMSERVER_OKBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, _ViewX + 149.0f, _ViewY + 72.0f, MSGBOX_BTN_WIDTH, MSGBOX_BTN_HEIGHT);
...
UIBlockedFromServer::~UIBlockedFromServer()
{

}

void UIBlockedFromServer::Update(mu_uint64 CurrentTime)
{

}

void UIBlockedFromServer::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UIBlockedFromServer::DrawFrame()
{
	_BackFrame->Draw();
}

void UIBlockedFromServer::DrawText()
{
	FONT_NORMAL->DrawStringC2D(TEXTINDEX::MESSAGEBOX_BLOCKEDFROMSERVER,
		...
		MU::MakeARGB(0, 0, 0, 0),
		_T("Your IP is blocked from the server"));
}

void UIBlockedFromServer::DrawButton()
{
	_OkButton->Draw();
	_OkButton->PostDraw();
}

mu_uint32 UIBlockedFromServer::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UIBlockedFromServer::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
