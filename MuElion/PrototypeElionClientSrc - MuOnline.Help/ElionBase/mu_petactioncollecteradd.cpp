...
MUPetActionCollecterAdd g_PetActionCollecterAdd;

MUPetActionCollecterAdd::MUPetActionCollecterAdd()
{
	m_isRooting = false;
...
mu_boolean MUPetActionCollecterAdd::Release(OBJECT* obj, CHARACTER *Owner)
{
...
mu_boolean MUPetActionCollecterAdd::Move(const mu_uint32 ThreadIndex, OBJECT* obj, CHARACTER *Owner, mu_int32 targetKey, mu_uint64 tick, mu_boolean bForceRender)
{
...
mu_boolean MUPetActionCollecterAdd::Effect(const mu_uint32 ThreadIndex, OBJECT* obj, CHARACTER *Owner, mu_int32 targetKey, mu_uint64 tick, mu_boolean bForceRender)
{
	...
	Vector(0.0f, 0.0f, 0.0f, vRelativePos);
	b->TransformPosition(ThreadIndex, ThreadInfo.BoneTransform[b->GetBoneIndex(7)], vRelativePos, Position, false);

	if (GetLargeRand(3) == 0)
	{
		Vector(0.6f, 1.0f, 0.4f, Light);
		CreateParticle(ThreadIndex, BITMAP_LIGHT + 3, Position, obj->Angle, Light, 1, 1.0f);
	}
...
mu_boolean MUPetActionCollecterAdd::Sound(const mu_uint32 ThreadIndex, OBJECT* obj, CHARACTER *Owner, mu_int32 targetKey, mu_uint64 tick, mu_boolean bForceRender)
{
	if (nullptr == obj || nullptr == Owner) return false;
...
void MUPetActionCollecterAdd::FindZen(OBJECT* obj)
{
...
mu_boolean MUPetActionCollecterAdd::CompTimeControl(const mu_uint64& dwCompTime, mu_uint64& dwTime)
{
