...
UIFailedConnect::UIFailedConnect(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_OkButton = new UIButton(MESSAGEBOX::MESSAGE_FAILEDCONNECT_OKBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, _ViewX + 149.0f, _ViewY + 72.0f, MSGBOX_BTN_WIDTH, MSGBOX_BTN_HEIGHT);
...
UIFailedConnect::~UIFailedConnect()
{

}

void UIFailedConnect::Update(mu_uint64 CurrentTime)
{

}

void UIFailedConnect::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UIFailedConnect::DrawFrame()
{
	_BackFrame->Draw();
}

void UIFailedConnect::DrawText()
{
	FONT_NORMAL->DrawStringC2D(TEXTINDEX::MESSAGEBOX_FAILEDCONNECT,
		...
		MU::MakeARGB(0, 0, 0, 0),
		_T("Failed to connect"));
}

void UIFailedConnect::DrawButton()
{
	_OkButton->Draw();
	_OkButton->PostDraw();
}

mu_uint32 UIFailedConnect::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UIFailedConnect::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
