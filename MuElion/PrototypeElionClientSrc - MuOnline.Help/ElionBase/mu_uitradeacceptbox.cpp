...
#define ACCEPT_POSY(y, h) (y + h - BUTTON_HEIGHT * 2.25f)
#define CANCEL_POSX (BOX_POSX + BOX_WIDTH - 98.0f - BUTTON_WIDTH)
#define CANCEL_POSY(y, h) (y + h - BUTTON_HEIGHT * 2.25f)

#define MAX_TEXTWIDTH (BOX_WIDTH * (180.0f / BASE_WIDTH))

#define TOP_TEXT 0.65f

UITradeAcceptBox::UITradeAcceptBox(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_AcceptButton = new UIButton(MESSAGEBOX::MESSAGE_TRADEACCEPT_ACCEPTBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, ACCEPT_POSX, ACCEPT_POSY(BOX_POSY, BOX_HEIGHT), BUTTON_WIDTH, BUTTON_HEIGHT);
		...
		_CancelButton = new UIButton(MESSAGEBOX::MESSAGE_TRADEACCEPT_CANCELBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CANCEL_POSX, CANCEL_POSY(BOX_POSY, BOX_HEIGHT), BUTTON_WIDTH, BUTTON_HEIGHT);
		...
		AddMessage(FONT_BOLD, GlobalText[371], MU::MakeARGB(255, 178, 0, 255));
		AddMessage(FONT_BOLD, GlobalText[372], MU::MakeARGB(255, 178, 0, 255));
		AddMessage(FONT_BOLD, GlobalText[373], MU::MakeARGB(255, 178, 0, 255));
		AddMessage(FONT_BOLD, GlobalText[374], MU::MakeARGB(255, 32, 32, 255));

		Configure();
	}
}

UITradeAcceptBox::~UITradeAcceptBox()
{
}

void UITradeAcceptBox::Configure()
{
	...
	_AcceptButton->SetPosition(ACCEPT_POSX, ACCEPT_POSY(posy, Height));
	_CancelButton->SetPosition(CANCEL_POSX, CANCEL_POSY(posy, Height));
}

void UITradeAcceptBox::Update(mu_uint64 CurrentTime)
{

}

void UITradeAcceptBox::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UITradeAcceptBox::DrawFrame()
{
	_Back->Draw();
	_Top->Draw();
	_Middle->Draw();
...
void UITradeAcceptBox::DrawText()
{
	...
	for (auto it = _Messages.begin(); it != _Messages.end(); ++it, ++index)
	{
		...
		data.font->DrawStringC2D(TEXTINDEX::MESSAGEBOX_TRADEACCEPT_BEGIN + index,
			messageX,
			messageY,
			1.0f,
			ETA_CENTER,
...
void UITradeAcceptBox::DrawButton()
{
	_AcceptButton->Draw();
	_CancelButton->Draw();
...
mu_uint32 UITradeAcceptBox::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UITradeAcceptBox::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
...
void UITradeAcceptBox::ClearMessages()
{
	_TextHeight = 0.0f;
	_Messages.clear();
}

void UITradeAcceptBox::AddMessage(MUTextSimpleRender *font, const mu_text *message, mu_uint32 color)
{
...
mu_float UITradeAcceptBox::SeparateText(MUTextSimpleRender *font, const mu_text *message, mu_uint32 color)
{
