...
UIPartyRequestBox::UIPartyRequestBox(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_AcceptButton = new UIButton(MESSAGEBOX::MESSAGE_PARTYREQUEST_ACCEPTBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, ACCEPT_POSX, ACCEPT_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
		...
		_CancelButton = new UIButton(MESSAGEBOX::MESSAGE_PARTYREQUEST_CANCELBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CANCEL_POSX, CANCEL_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
...
UIPartyRequestBox::~UIPartyRequestBox()
{

}

void UIPartyRequestBox::Update(mu_uint64 CurrentTime)
{

}

void UIPartyRequestBox::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UIPartyRequestBox::DrawFrame()
{
	_BackFrame->Draw();
}

void UIPartyRequestBox::DrawText()
{
	mu_text buffer[256] = { 0 };
	mu_sprintf_s(buffer, mu_countof(buffer), _T("") SPF_STRING _T("\n") SPF_STRING _T(""), CharactersClient[FindCharacterIndex(_PlayerKey)].ID, GlobalText[425]);

	FONT_NORMAL->DrawStringC2D(TEXTINDEX::MESSAGEBOX_PARTYREQUEST,
...
void UIPartyRequestBox::DrawButton()
{
	_AcceptButton->Draw();
	_CancelButton->Draw();
...
mu_uint32 UIPartyRequestBox::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UIPartyRequestBox::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
