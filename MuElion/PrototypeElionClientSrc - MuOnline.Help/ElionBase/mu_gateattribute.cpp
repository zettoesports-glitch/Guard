...
GATE_ATTRIBUTE GateAttribute[MAX_GATES];
...
mu_boolean OpenGateScript(const mu_text *filename)
{
	...
	if (compressedHeader->DecompressedSize != sizeof(GateAttribute))
	{
		delete[] dataBuffer;
		return false;
	}
	...
	mu_memcpy(GateAttribute, decompressedBuffer, sizeof(GateAttribute));
