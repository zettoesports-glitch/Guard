...
UIStorageWithdrawBox::UIStorageWithdrawBox(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_GoldInput = new UIEditBox(MESSAGEBOX::MESSAGE_STORAGEWITHDRAW_GOLDINPUT, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, BOX_POSX + (BOX_WIDTH - GOLDINPUT_WIDTH) * 0.5f, BOX_POSY + MSGBOX_TOP_HEIGHT - 16.0f, GOLDINPUT_WIDTH, GOLDINPUT_HEIGHT);
		...
		_AcceptButton = new UIButton(MESSAGEBOX::MESSAGE_STORAGEWITHDRAW_ACCEPTBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, ACCEPT_POSX, ACCEPT_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
		...
		_CancelButton = new UIButton(MESSAGEBOX::MESSAGE_STORAGEWITHDRAW_CANCELBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CANCEL_POSX, CANCEL_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
...
UIStorageWithdrawBox::~UIStorageWithdrawBox()
{

}

void UIStorageWithdrawBox::Update(mu_uint64 CurrentTime)
{
	_GoldInput->Update(CurrentTime);
}

void UIStorageWithdrawBox::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UIStorageWithdrawBox::DrawFrame()
{
	_BackFrame->Draw();
	_InputBack->Draw();
}

void UIStorageWithdrawBox::DrawText()
{
	FONT_NORMAL->DrawStringC2D(TEXTINDEX::STORAGE_WITHDRAWBOX,
...
void UIStorageWithdrawBox::DrawButton()
{
	_AcceptButton->Draw();
	_CancelButton->Draw();
...
mu_uint32 UIStorageWithdrawBox::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UIStorageWithdrawBox::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
