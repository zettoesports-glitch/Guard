...
UIToggleButton::UIToggleButton(mu_uint32 objectId, UISceneManager *manager, UIBase *owner, mu_float aspectWidth, mu_float aspectHeight, mu_float x, mu_float y, mu_float width, mu_float height, mu_uint32 flag, mu_uint32 event) : UIRenderTextureBase()
{
	if (manager != nullptr)
	{
...
void UIToggleButton::SetStateData(const mu_uint32 index, const UI_STATE_DATA &state)
{
	_UpdateGPU = true;
	mu_memcpy(&_States[index], &state, sizeof(state));
}

void UIToggleButton::SetStateText(const mu_uint32 index, MUTextSimpleRender *font, const mu_text *text, mu_uint32 textColor, mu_uint32 backColor, mu_float marginX, mu_float marginY)
{
...
void UIToggleButton::ChangeState(mu_uint32 state)
{
	_UpdateGPU = true;
	_CurrentState = state;
...
mu_uint32 UIToggleButton::GetState()
{
	return _CurrentState;
}

mu_boolean UIToggleButton::IsPressed()
{
	return (_CurrentState == UI_PRESSED || _CurrentState == UI_PRESSED_OVER);
}

void UIToggleButton::SetTooltip(MUTextSimpleRender *fontIndex, const mu_text *message, mu_uint32 messageColor, mu_uint32 backgroundColor, mu_boolean isTop)
{
...
void UIToggleButton::OnMouseOver(mu_int32 mouse_x, mu_int32 mouse_y)
{
...
void UIToggleButton::OnMouseLeave()
{
...
void UIToggleButton::OnMousePressed(mu_uint32 mouse_flags, mu_uint32 double_flags, mu_uint32 virtual_keys)
{
...
void UIToggleButton::OnMouseReleased(mu_uint32 mouse_flags, mu_uint32 virtual_keys)
{
	if (_CurrentState == UI_DISABLED)
	{
		return;
...
void UIToggleButton::OnMouseCancelled(mu_uint32 mouse_flags)
{
	if (_CurrentState == UI_DISABLED)
	{
		return;
