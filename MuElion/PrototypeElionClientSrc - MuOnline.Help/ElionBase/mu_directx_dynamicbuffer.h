#ifndef __MU_DIRECTX_DYNAMICBUFFER_H__
#define __MU_DIRECTX_DYNAMICBUFFER_H__

#pragma once

namespace EDirectXUtils
{
	...
	template<class T, const D3D11_BIND_FLAG BindFlags, mu_uint32 InitialSize, mu_uint32 GrowSize>
	...
	public:
		...
		EINLINE const mu_boolean Initialize(const mu_uint32 SwapchainCount)
		{
			...
			_Buffers.resize(SwapchainCount, nullptr);
		...
		EINLINE void UpdateBuffer()
		{
			...
			if (disabledTransfers == false && _BuffersResize[currentIndex] == true)
			{
				...
				bufferDesc.ByteWidth = _StagingCount * sizeof(T);
				bufferDesc.Usage = D3D11_USAGE_DEFAULT;
				bufferDesc.BindFlags = BindFlags;
				bufferDesc.CPUAccessFlags = 0;

				D3D11_SUBRESOURCE_DATA initialData = {};
				initialData.pSysMem = _BufferMemory;

				ID3D11Buffer *newBuffer = nullptr;
				HRESULT result = EDirectXInit::Device->CreateBuffer(&bufferDesc, &initialData, &newBuffer);
				mu_assert(SUCCEEDED(result));
				if (SUCCEEDED(result))
				{
					EDirectXHelpers::Release(_Buffers[currentIndex]);
					_Buffers[currentIndex] = newBuffer;
			...
			{
				ID3D11Buffer *buffer = _Buffers[currentIndex];

				for (auto it = copies.Data.begin(); it != copies.Data.end(); ++it)
				{
					EMappedMemoryRange &tmp = *it;

					D3D11_BOX destBox = { static_cast<mu_uint32>(tmp.offset), 0, 0, static_cast<mu_uint32>(tmp.offset + tmp.size), 1, 1 };
					EDirectXInit::DeviceContext->UpdateSubresource(buffer, 0, &destBox, reinterpret_cast<mu_uint8*>(_BufferMemory) + tmp.offset, 0, 0);
				}
				copies.Data.clear();
			}
		}
		...
		EINLINE ID3D11Buffer *RetrieveBuffer(const mu_uint32 index)
		{
