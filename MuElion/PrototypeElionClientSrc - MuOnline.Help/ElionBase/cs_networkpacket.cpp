...
mu_boolean CSNetworkPacket::IsValid()
{
...
mu_uint32 CSNetworkPacket::ParsePacket(mu_uint8 *buffer, mu_uint32 bufferSize)
{
	...
	if (IsMissing() == true)
	{
		...
		if (IsMissing() == false)
		{
			g_ConnectServerCrypt.Decode(GetBuffer() + PACKET_MINIMUM_SIZE, GetBuffer() + PACKET_MINIMUM_SIZE, GetPacketSize() - PACKET_MINIMUM_SIZE);
		}
	}
