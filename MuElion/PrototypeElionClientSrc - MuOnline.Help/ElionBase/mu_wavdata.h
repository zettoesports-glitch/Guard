#ifndef __MU_WAV_DATA_H__
#define __MU_WAV_DATA_H__
...
typedef struct waveformat_tag {
	mu_uint16    wFormatTag;        /* format type */
	mu_uint16    nChannels;         /* number of channels (i.e. mono, stereo...) */
	mu_uint32    nSamplesPerSec;    /* sample rate */
	mu_uint32    nAvgBytesPerSec;   /* for buffer estimation */
	mu_uint16    nBlockAlign;       /* block size of data */
} WAVEFORMAT;

typedef struct pcmwaveformat_tag {
	WAVEFORMAT  wf;
	mu_uint16   wBitsPerSample;
} PCMWAVEFORMAT;
...
typedef struct tWAVEFORMATEX
{
	...
	mu_uint32   nAvgBytesPerSec;   /* for buffer estimation */
	mu_uint16    nBlockAlign;       /* block size of data */
	mu_uint16    wBitsPerSample;    /* Number of bits per sample of mono data */
	mu_uint16    cbSize;            /* The count in bytes of the size of
...
typedef struct {
	WAVEFORMATEX    Format;
	union {
		mu_uint16 wValidBitsPerSample;       /* bits of precision  */
		mu_uint16 wSamplesPerBlock;          /* valid if wBitsPerSample==0 */
		mu_uint16 wReserved;                 /* If neither applies, set to zero. */
	} Samples;
	mu_uint32           dwChannelMask;      /* which channels are */
											/* present in stream  */
	GUID            SubFormat;
...
#pragma pack(push,1)
struct ARM_PACKED RIFFChunk
{
	mu_uint32 tag;
	mu_uint32 size;
};

struct ARM_PACKED RIFFChunkHeader
{
	mu_uint32 tag;
	mu_uint32 size;
	mu_uint32 riff;
...
struct ARM_PACKED DLSLoop
{
	static const mu_uint32 LOOP_TYPE_FORWARD = 0x00000000;
	static const mu_uint32 LOOP_TYPE_RELEASE = 0x00000001;

	mu_uint32 size;
	mu_uint32 loopType;
	mu_uint32 loopStart;
	mu_uint32 loopLength;
};

struct ARM_PACKED RIFFDLSSample
{
	static const mu_uint32 OPTIONS_NOTRUNCATION = 0x00000001;
	static const mu_uint32 OPTIONS_NOCOMPRESSION = 0x00000002;
...
struct ARM_PACKED MIDILoop
{
	static const mu_uint32 LOOP_TYPE_FORWARD = 0x00000000;
	static const mu_uint32 LOOP_TYPE_ALTERNATING = 0x00000001;
	static const mu_uint32 LOOP_TYPE_BACKWARD = 0x00000002;

	mu_uint32 cuePointId;
	mu_uint32 type;
	mu_uint32 start;
	mu_uint32 end;
	mu_uint32 fraction;
...
struct ARM_PACKED RIFFMIDISample
{
	mu_uint32        manufacturerId;
	mu_uint32        productId;
	mu_uint32        samplePeriod;
	mu_uint32        unityNode;
	mu_uint32        pitchFraction;
	mu_uint32        SMPTEFormat;
	mu_uint32        SMPTEOffset;
	mu_uint32        loopCount;
	mu_uint32        samplerData;
};
#pragma pack(pop)

#define SOUND_KEEPTIME (5 * 60 * 1000) // 5 Minutes
...
public:
	...
	void SetWavPath(const mu_string Path);
	mu_boolean LoadWav();
	void Unload();
...
private:
	const RIFFChunk *FindChunk(const mu_uint8 *Data, mu_uint32 DataSize, mu_uint32 Tag);
	mu_boolean FindFormatAndData();

public:
	EINLINE mu_boolean IsFailed()
	{
		return _Failed;
	}

	EINLINE mu_int32 GetReferenceCount()
	{
	...
	EINLINE void DecreaseReference()
	{
		if (_ReferenceCount.fetch_sub(1) == 1)
		{
			_LastReferenceTime = g_CurrentTime + SOUND_KEEPTIME;
		}
	}

	EINLINE mu_uint64 GetUnloadTime()
	{
...
public:
	...
	mu_uint32 _WavBytes;
	mu_uint32 _AudioBytes;

	const WAVEFORMATEX *_WavFormat;
	mu_uint8 *_WavBuffer;
	const mu_uint8 *_AudioBuffer;

	mu_string _WavPath;
	mu_atomic_bool _VoiceLocked;
	mu_atomic_int32_t _ReferenceCount;
	mu_uint64 _LastReferenceTime;
	mu_uint32 _VoiceIndex;
};
