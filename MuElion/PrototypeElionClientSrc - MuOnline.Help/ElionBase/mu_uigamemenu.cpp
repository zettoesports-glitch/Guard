...
#define CHARACTER_WIDTH 108.0f
#define CHARACTER_HEIGHT 30.0f
#define CHARACTER_POSX (WINDOW_POSX + (WINDOW_WIDTH - EXIT_WIDTH) * 0.5f)
#define CHARACTER_POSY (WINDOW_POSY + 33.0f + (CHARACTER_HEIGHT + 4.0f) * 2.0f)

#define OPTION_WIDTH 108.0f
#define OPTION_HEIGHT 30.0f
#define OPTION_POSX (WINDOW_POSX + (WINDOW_WIDTH - EXIT_WIDTH) * 0.5f)
#define OPTION_POSY (WINDOW_POSY + 33.0f + (OPTION_HEIGHT + 4.0f) * 3.0f)

#define CLOSE_WIDTH 108.0f
#define CLOSE_HEIGHT 30.0f
#define CLOSE_POSX (WINDOW_POSX + (WINDOW_WIDTH - EXIT_WIDTH) * 0.5f)
#define CLOSE_POSY (WINDOW_POSY + 33.0f + (OPTION_HEIGHT + 4.0f) * 4.0f)

UIGameMenu::UIGameMenu(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_ExitButton = new UIButton(OPTIONUI::MESSAGE_GAME_EXITBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, EXIT_POSX, EXIT_POSY, EXIT_WIDTH, EXIT_HEIGHT);
		...
		_ServerButton = new UIButton(OPTIONUI::MESSAGE_GAME_SERVERBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, SERVER_POSX, SERVER_POSY, SERVER_WIDTH, SERVER_HEIGHT);
		...
		_CharacterButton = new UIButton(OPTIONUI::MESSAGE_GAME_CHARACTERBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CHARACTER_POSX, CHARACTER_POSY, CHARACTER_WIDTH, CHARACTER_HEIGHT);
		_CharacterButton->SetBlendState(EBlendState::eAlphaSrc_InvAlphaSrc);
		_CharacterButton->SetStateData(UIButton::UI_DISABLED, UI_STATE_DATA(&INTERFACES::Data[INTERFACES::MENU_BUTTON], EVector2(0.0f, 0.0f), EVector2(1.0f, 0.25f), MU::MakeARGB(100, 100, 100, 255)));
		_CharacterButton->SetStateData(UIButton::UI_NORMAL, UI_STATE_DATA(&INTERFACES::Data[INTERFACES::MENU_BUTTON], EVector2(0.0f, 0.0f), EVector2(1.0f, 0.25f), MU::MakeARGB(255, 255, 255, 255)));
		_CharacterButton->SetStateData(UIButton::UI_OVER, UI_STATE_DATA(&INTERFACES::Data[INTERFACES::MENU_BUTTON], EVector2(0.0f, 0.25f), EVector2(1.0f, 0.5f), MU::MakeARGB(255, 255, 255, 255)));
		_CharacterButton->SetStateData(UIButton::UI_PRESSED, UI_STATE_DATA(&INTERFACES::Data[INTERFACES::MENU_BUTTON], EVector2(0.0f, 0.5f), EVector2(1.0f, 0.75f), MU::MakeARGB(255, 255, 255, 255)));
		_CharacterButton->SetStateText(UIButton::UI_DISABLED, FONT_NORMAL, GlobalText[383], MU::MakeARGB(150, 150, 150, 200));
		_CharacterButton->SetStateText(UIButton::UI_NORMAL, FONT_NORMAL, GlobalText[383], MU::MakeARGB(255, 255, 255, 255));
		_CharacterButton->SetStateText(UIButton::UI_OVER, FONT_NORMAL, GlobalText[383], MU::MakeARGB(255, 255, 255, 255));
		_CharacterButton->SetStateText(UIButton::UI_PRESSED, FONT_NORMAL, GlobalText[383], MU::MakeARGB(255, 255, 255, 255));

		_OptionButton = new UIButton(OPTIONUI::MESSAGE_GAME_OPTIONBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, OPTION_POSX, OPTION_POSY, OPTION_WIDTH, OPTION_HEIGHT);
		...
		_CloseButton = new UIButton(OPTIONUI::MESSAGE_GAME_CLOSEBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CLOSE_POSX, CLOSE_POSY, CLOSE_WIDTH, CLOSE_HEIGHT);
...
void UIGameMenu::Update(mu_uint64 CurrentTime)
{

}

void UIGameMenu::Draw()
{
	_BackFrame->Draw();
	_TopFrame->Draw();
	_BottomFrame->Draw();
	...
	_ExitButton->Draw();
	_ServerButton->Draw();
	_CharacterButton->Draw();
	_OptionButton->Draw();
	_CloseButton->Draw();
}

void UIGameMenu::PostDraw()
{
	_ExitButton->PostDraw();
	_ServerButton->PostDraw();
	_CharacterButton->PostDraw();
	_OptionButton->PostDraw();
	_CloseButton->PostDraw();
}

UIBase *UIGameMenu::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
	...
	if (focus != nullptr)
	{
		...
		object = _CharacterButton->GetFocus(mouseX, mouseY);
