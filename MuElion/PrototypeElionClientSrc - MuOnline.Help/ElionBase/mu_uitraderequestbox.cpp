...
UITradeRequestBox::UITradeRequestBox(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_AcceptButton = new UIButton(MESSAGEBOX::MESSAGE_TRADEREQUEST_ACCEPTBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, ACCEPT_POSX, ACCEPT_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
		...
		_CancelButton = new UIButton(MESSAGEBOX::MESSAGE_TRADEREQUEST_CANCELBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CANCEL_POSX, CANCEL_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
...
UITradeRequestBox::~UITradeRequestBox()
{

}

void UITradeRequestBox::Update(mu_uint64 CurrentTime)
{

}

void UITradeRequestBox::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UITradeRequestBox::DrawFrame()
{
	_BackFrame->Draw();
}

void UITradeRequestBox::DrawText()
{
	mu_text buffer[256] = { 0 };
	mu_sprintf_s(buffer, mu_countof(buffer), _T("") SPF_STRING _T("\n") SPF_STRING _T(""), CharactersClient[FindCharacterIndex(_PlayerKey)].ID, GlobalText[419]);

	FONT_NORMAL->DrawStringC2D(TEXTINDEX::MESSAGEBOX_TRADEREQUEST,
...
void UITradeRequestBox::DrawButton()
{
	_AcceptButton->Draw();
	_CancelButton->Draw();
...
mu_uint32 UITradeRequestBox::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UITradeRequestBox::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
