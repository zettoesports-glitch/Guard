...
UIGuildLeaveBox::UIGuildLeaveBox(UISceneManager *manager) : UIBase()
{
	if (manager != nullptr)
	{
	...
	// Initialize
	{
		...
		_AcceptButton = new UIButton(MESSAGEBOX::MESSAGE_GUILDLEAVE_ACCEPTBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, ACCEPT_POSX, ACCEPT_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
		...
		_CancelButton = new UIButton(MESSAGEBOX::MESSAGE_GUILDLEAVE_CANCELBUTTON, manager, this, SCREEN_WIDTH, SCREEN_HEIGHT, CANCEL_POSX, CANCEL_POSY, BUTTON_WIDTH, BUTTON_HEIGHT);
...
UIGuildLeaveBox::~UIGuildLeaveBox()
{

}

void UIGuildLeaveBox::Prepare(GUILD_LIST_t *member)
{
	mu_text buffer[4 * 1024] = { 0 };
	MARK_t &guildMark = GuildMark[Hero->GuildMarkIndex];

	if (member != nullptr)
	{
		_IsKickMember = true;
		_MemberToKick = *member;
		mu_sprintf_s(buffer, mu_countof(buffer), GlobalText[1367], member->Name.c_str());
		mu_strcat_s(buffer, mu_countof(buffer), _T("\n"));
		mu_strcat_s(buffer, mu_countof(buffer), GlobalText[1369]);
	}
	else
	{
		_IsKickMember = false;
		...
		{
			if (guildMark.UnionName.size() == 0)
			{
				mu_sprintf_s(buffer, mu_countof(buffer), _T("") SPF_STRING _T(""), GlobalText[1366]);
			}
			else
			{
				mu_sprintf_s(buffer, mu_countof(buffer), _T("") SPF_STRING _T("\n") SPF_STRING _T("\n") SPF_STRING _T("\n") SPF_STRING _T(""), GlobalText[1363], GlobalText[1364], GlobalText[1365], GlobalText[1366]);
			}
		}
		else
		{
...
void UIGuildLeaveBox::Update(mu_uint64 CurrentTime)
{

}

void UIGuildLeaveBox::PostDraw()
{
	DrawFrame();
	DrawText();
	DrawButton();
...
void UIGuildLeaveBox::DrawFrame()
{
	_Back->Draw();
	_Top->Draw();
	_Middle->Draw();
...
void UIGuildLeaveBox::DrawText()
{
	FONT_NORMAL->DrawStringC2D(TEXTINDEX::MESSAGEBOX_GUILDLEAVE,
...
void UIGuildLeaveBox::DrawButton()
{
	_AcceptButton->Draw();
	_CancelButton->Draw();
...
mu_uint32 UIGuildLeaveBox::OnPreKeyPressed(const SDL_Keysym &key)
{
	switch (key.scancode)
	{
...
UIBase *UIGuildLeaveBox::GetFocus(mu_int32 mouseX, mu_int32 mouseY)
{
